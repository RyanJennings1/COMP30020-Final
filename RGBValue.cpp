...
RGBValue RGBValue::modulate(RGBValue &color) {
  // convert to float
  float u[3] = {static_cast<float>(red / 255), static_cast<float>(green / 255), static_cast<float>(blue / 255)};
  float v[3] = {static_cast<float>(color.red / 255), static_cast<float>(color.green / 255),
                static_cast<float>(color.blue / 255)};

  // return a new RGBValue
  return RGBValue(static_cast<Byte>(255 * u[0] * v[0]), static_cast<Byte>(255 * u[1] * v[1]),
                  static_cast<Byte>(255 * u[2] * v[2]));
}
...
const RGBValue operator+(const RGBValue &left, const RGBValue &right) {
  ...
  if (blue > 255)
    blue = 255;
  return RGBValue(static_cast<Byte>(red), static_cast<Byte>(green), static_cast<Byte>(blue));
}
...
const RGBValue operator*(float scalar, const RGBValue &colour) {
  return RGBValue(static_cast<Byte>(scalar * colour.red), static_cast<Byte>(scalar * colour.green),
                  static_cast<Byte>(scalar * colour.blue));
}
